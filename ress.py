...
Required files (all in same folder as this script):
  cleaned_jobs.csv        — dataset (label: 0=Real, 1=Fake)
  pac_model.pkl           — trained PassiveAggressiveClassifier (classes: 0,1,2)
  tfidf_vectorizer.pkl    — fitted TfidfVectorizer (5000 features)
...
IRRELEVANT_JOBS = [
    {
        ...
        "company_profile": "CosmoGuide Inc.",
    },
    {
        ...
        "company_profile": "StreamRate Inc.",
    },
    {
        ...
        "requirements": "Survival instinct. Zero fear of the undead.",
        "company_profile": "ZombiePrep LLC.",
    },
    {
        ...
        "description": "Collect regolith samples on the lunar surface. Must tolerate 2-week communication blackouts.",
        "requirements": "Space suit provided. Must hold breath 8 seconds minimum.",
        "company_profile": "LunarMineCo.",
    },
    {
...
:root{
  ...
  --bg:#f5f3ef; --card:#ffffff; --border:#e2ddd6; --text:#1c1a17; --muted:#6b6560;
}
...
.hero h1{font-family:'Fraunces',serif;font-size:clamp(2rem,4.5vw,3.2rem);font-weight:900;
  line-height:1.08;margin:0 0 .6rem;}
.hero h1 em{font-style:italic;color:var(--amber);}
.hero p{opacity:.85;font-size:1rem;max-width:540px;line-height:1.7;margin:0;}
.hero-pills{display:flex;gap:.7rem;flex-wrap:wrap;margin-top:1.4rem;}
.hero-pill{background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.18);
  border-radius:50px;padding:.3rem .9rem;font-size:.78rem;color:#e2ddd6;}
...
.stat-card .l{font-size:.75rem;color:var(--muted);margin-top:.25rem;letter-spacing:.02em;text-transform:uppercase;}
...
.b-real{background:#dcfce7;color:var(--real);border:1px solid #86efac;border-radius:50px;padding:.14rem .65rem;font-size:.71rem;font-weight:700;}
.b-fake{background:#fee2e2;color:var(--fake);border:1px solid #fca5a5;border-radius:50px;padding:.14rem .65rem;font-size:.71rem;font-weight:700;}
.b-irr{background:#fef9c3;color:var(--irr);border:1px solid #fde047;border-radius:50px;padding:.14rem .65rem;font-size:.71rem;font-weight:700;}
...
.box-info{background:#eff6ff;border-left:4px solid #3b82f6;border-radius:10px;padding:.85rem 1.1rem;margin-bottom:1rem;font-size:.89rem;}
.box-ok{background:#f0fdf4;border-left:4px solid var(--real);border-radius:10px;padding:.85rem 1.1rem;margin-bottom:1rem;font-size:.89rem;}
.box-warn{background:#fef2f2;border-left:4px solid var(--fake);border-radius:10px;padding:.85rem 1.1rem;margin-bottom:1rem;font-size:.89rem;}
.box-ai{background:#fffbeb;border-left:4px solid var(--amber);border-radius:10px;padding:.85rem 1.1rem;margin-bottom:1rem;font-size:.89rem;}
.box-irr{background:#fefce8;border-left:4px solid var(--irr);border-radius:10px;padding:.85rem 1.1rem;margin-bottom:1rem;font-size:.89rem;}

.detail-card{background:var(--card);border-radius:18px;padding:2.2rem;border:1px solid var(--border);box-shadow:0 6px 24px rgba(15,39,68,.09);}
...
div.stButton>button{background:var(--navy2)!important;color:#fff!important;border:none!important;
  border-radius:10px!important;font-weight:600!important;transition:background .18s,transform .15s!important;}
...
# ─────────────────────────────────────────────────────────────────────────────
# ML ENGINE — loads existing pac_model.pkl + tfidf_vectorizer.pkl
# ─────────────────────────────────────────────────────────────────────────────
@st.cache_resource(show_spinner="🛡️ Loading PAC model and dataset…")
def build_ml_engine():
    """
    1. Load cleaned_jobs.csv
    2. Load tfidf_vectorizer.pkl  (fitted TfidfVectorizer, 5000 features)
    3. Load pac_model.pkl         (PassiveAggressiveClassifier, classes 0/1/2)
    4. Pre-classify every CSV row  → df['pac_pred']
    5. Build a second TF-IDF for cosine-similarity search
    """
    ...
    pac_vec = joblib.load(VEC_PATH)    # TfidfVectorizer
    model   = joblib.load(MODEL_PATH)  # PassiveAggressiveClassifier

    # ── 3. Build combined text exactly as tfidf_features.py does ──────────
    combined = df[TEXT_COLS].agg(" ".join, axis=1)

    # ── 4. Transform with saved vectorizer → predict ───────────────────────
    X = pac_vec.transform(combined)
    df["pac_pred"] = model.predict(X).astype(int)

    # ── 5. Second TF-IDF for cosine-similarity search ──────────────────────
...
def classify_job(text_dict: dict, pac_vec, model) -> int:
    """
    Classify a single job dict → 0 Real | 1 Fake | 2 Irrelevant.
    Mirrors tfidf_features.py:transform_single() exactly.
    """
    def _clean(t):
...
def fraud_pill(val: int) -> str:
    if val == 0:
        return "<span class='fpill fp-real'>🟢 fraudulent = 0 → Real Job</span>"
    if val == 1:
        return "<span class='fpill fp-fake'>🔴 fraudulent = 1 → Fake Job</span>"
...
def co_login(email, pw):
    c = _conn()
    r = c.execute("SELECT * FROM companies WHERE email=? AND password_hash=?", (email, _hp(pw))).fetchone()
...
def sk_login(email, pw):
    c = _conn()
    r = c.execute("SELECT * FROM seekers WHERE email=? AND password_hash=?", (email, _hp(pw))).fetchone()
    c.close(); return dict(r) if r else None
...
def profile_score(s):
    return int(sum(1 for f in ["name","phone","skills","bio","preferred_location","expected_salary"] if s.get(f)) / 6 * 100)

# ── Posted Jobs ───────────────────────────────────────────────────────────────
def job_post(cid, title, jtype, loc, salary, exp, desc, req, mobile):
    c = _conn()
    c.execute(
        "INSERT INTO jobs(company_id,title,job_type,location,salary_range,experience_required,description,requirements,contact_mobile) VALUES(?,?,?,?,?,?,?,?,?)",
        (cid, title, jtype, loc, salary, exp, desc, req, mobile))
    c.commit(); c.close()

def jobs_get(q="", location="", limit=200):
    c = _conn()
    sql = ("SELECT j.*,co.name AS company_name FROM jobs j "
           "JOIN companies co ON j.company_id=co.id WHERE 1=1")
    p = []
...
def job_get(jid):
    ...
    r = c.execute(
        ...
        "JOIN companies co ON j.company_id=co.id WHERE j.id=?", (jid,)).fetchone()
    c.close(); return dict(r) if r else None

def co_jobs(cid):
    c = _conn()
    rows = c.execute(
        "SELECT j.*,COUNT(a.id) AS appl FROM jobs j "
        "LEFT JOIN applications a ON a.job_id=j.id "
        "WHERE j.company_id=? GROUP BY j.id ORDER BY j.created_at DESC", (cid,)).fetchall()
    c.close(); return [dict(r) for r in rows]

def job_delete(jid, cid):
    c = _conn()
    c.execute("DELETE FROM applications WHERE job_id=?", (jid,))
    c.execute("DELETE FROM jobs WHERE id=? AND company_id=?", (jid, cid))
    c.commit(); c.close()

def co_applicants(cid, jid=None):
    c = _conn()
    sql = ("SELECT s.name,s.email,s.skills,s.experience,a.applied_at,a.status,j.title AS job_title "
           "FROM applications a JOIN seekers s ON s.id=a.seeker_id "
           "JOIN jobs j ON j.id=a.job_id WHERE j.company_id=?")
    p = [cid]
    if jid: sql += " AND a.job_id=?"; p.append(jid)
    rows = c.execute(sql + " ORDER BY a.applied_at DESC", p).fetchall()
    c.close(); return [dict(r) for r in rows]

def co_stats(cid):
    c = _conn()
    t = c.execute("SELECT COUNT(*) FROM jobs WHERE company_id=?", (cid,)).fetchone()[0]
    a = c.execute(
...
def ds_apply(idx, sid):
    c = _conn()
    try:
        c.execute("INSERT INTO ds_applications(ds_idx,seeker_id) VALUES(?,?)", (idx, sid))
...
def ds_applied(idx, sid):
    c = _conn()
    r = c.execute("SELECT 1 FROM ds_applications WHERE ds_idx=? AND seeker_id=?", (idx, sid)).fetchone()
    c.close(); return bool(r)

def ds_my_apps(sid):
    c = _conn()
    rows = c.execute(
        "SELECT * FROM ds_applications WHERE seeker_id=? ORDER BY applied_at DESC", (sid,)).fetchall()
    c.close(); return [dict(r) for r in rows]

def sk_stats(sid):
    c = _conn()
    a = c.execute("SELECT COUNT(*) FROM applications WHERE seeker_id=?", (sid,)).fetchone()[0]
    b = c.execute("SELECT COUNT(*) FROM ds_applications WHERE seeker_id=?", (sid,)).fetchone()[0]
    c.close(); return a + b, b
...
def navbar():
    ...
    c0.markdown(
        ...
        'font-weight:900;color:#0f2744;">🛡️ True'
        '<em style="color:#e8a020;font-style:italic;">Hire</em></span>',
        unsafe_allow_html=True)
    if c1.button("Home", key="n_home"): go("home")
    if c2.button("Jobs", key="n_jobs"):
        st.session_state.sel_job = None; go("jobs")
    if u:
    ...
    st.markdown(
        "<hr style='border:none;border-top:1px solid #e2ddd6;margin:.2rem 0 1rem;'>",
        unsafe_allow_html=True)
...
def page_home():
    navbar()
    df, pac_vec, model, search_vec, search_mat = build_ml_engine()
    ...
    <div class="hero">
      ...
      <p>PAC model loaded from <code>pac_model.pkl</code> classifies every listing using the
         <b>fraudulent</b> label — 0 = Real, 1 = Fake, 2 = Irrelevant.</p>
      <div class="hero-pills">
        ...
        <div class="hero-pill"><strong>tfidf_vectorizer.pkl</strong> loaded</div>
      </div>
    ...
    if c3.button("🔍 Search", use_container_width=True):
        ...
        st.session_state.sl = loc
        go("jobs")

    # ── Live Classifier ─────────────────────────────────────────────────────
    st.markdown('<p class="sec-title" style="margin-top:1.6rem;">🔬 Live Job Classifier</p>', unsafe_allow_html=True)
    st.markdown('<p class="sec-sub">Paste any job description — the loaded PAC model classifies it instantly</p>', unsafe_allow_html=True)
    with st.expander("▶ Try the classifier", expanded=False):
        lt = st.text_area("Job title + description", height=110,
                           placeholder="Earn money fast! No investment needed. Work from home, guaranteed income…")
        ls = st.text_input("Salary range (optional)", placeholder="e.g. ₹5000/week, Free registration")
        lc = st.text_input("Company profile (optional)", placeholder="Brief company description")
        if st.button("⚡ Classify Now", key="live_cls"):
            if lt.strip():
                cls = classify_job(
                    {"title": lt[:120], "description": lt,
                     "salary_range": ls, "company_profile": lc},
                    pac_vec, model)
                bg    = {"Real":"#dcfce7", "Fake":"#fee2e2", "Irrelevant":"#fef9c3"}[CLASS_LABEL[cls]]
                icon  = {"Real":"✅", "Fake":"⚠️", "Irrelevant":"❓"}[CLASS_LABEL[cls]]
                st.markdown(
                    f'<div style="background:{bg};border-radius:12px;padding:1rem 1.4rem;'
                    f'font-size:1rem;font-weight:700;margin-top:.5rem;">'
                    f'{icon} PAC Model → <b>{CLASS_LABEL[cls]}</b> '
    ...
    st.markdown('<p class="sec-sub">Sampled from cleaned_jobs.csv — fraudulent value (0/1) and PAC prediction shown for each</p>', unsafe_allow_html=True)
        ...
        with cols[i % 2]:
            ...
            <div class="jcard">
              ...
              <div style="font-size:.7rem;color:#9ca3af;margin-top:.3rem;">
                Dataset fraudulent value: <b>{csv_lbl}</b> &nbsp;|&nbsp; PAC model: <b>{CLASS_LABEL[cls]}</b>
              </div>
            </div>""", unsafe_allow_html=True)
            if st.button("View Details", key=f"hf_{idx}"):
    ...
    ca, cb = st.columns(2)
    with ca:
        st.markdown("""<div style="background:#0f2744;border-radius:16px;padding:1.8rem;color:#fff;">
        <h3 style="font-family:'Fraunces',serif;color:#fff;margin-bottom:.4rem;">🏢 Hiring?</h3>
        <p style="color:#93c5fd;margin-bottom:1rem;">Post jobs. PAC model auto-classifies every listing.</p>
        </div>""", unsafe_allow_html=True)
    ...
    with cb:
        ...
        <h3 style="font-family:'Fraunces',serif;color:#0f2744;margin-bottom:.4rem;">🔍 Job Hunting?</h3>
...
def page_jobs():
    ...
    if st.session_state.sel_job:
        ...
        if src == "dataset":
            detail_dataset(df, idx, pac_vec, model)
        elif src == "irrelevant":
            detail_irrelevant(IRRELEVANT_JOBS[idx])
        elif src == "posted":
            j = job_get(idx)
    ...
    with st.expander("🔍 Search & Filter", expanded=True):
        ...
        show = c3.selectbox("Filter by label",
                             ["All", "Real Only (label=0)", "Fake Only (label=1)", "Irrelevant Only"])
        if st.button("🔍 Apply Search", use_container_width=True):
            st.session_state.sq = q
            st.session_state.sl = loc

    # TF-IDF ranked search on dataset
    ...
    total = (len(results) + len(posted) +
             (len(IRRELEVANT_JOBS) if show in ("All","Irrelevant Only") else 0))
    st.markdown(
        f"**{total} listing(s)** &nbsp;"
        f"<span class='b-ai'>🤖 TF-IDF ranked · PAC classified</span>",
        unsafe_allow_html=True)

    st.markdown("""<div class="box-ai" style="font-size:.81rem;margin-bottom:.9rem;">
      <b>Classification key:</b> &nbsp;
      <span class="b-real">✔ Real</span> → PAC/fraudulent = 0 &nbsp;|&nbsp;
      <span class="b-fake">✘ Fake</span> → PAC/fraudulent = 1 &nbsp;|&nbsp;
      <span class="b-irr">? Irrelevant</span> → PAC = 2
      &nbsp;&nbsp; <b>Source model: pac_model.pkl</b>
    </div>""", unsafe_allow_html=True)
    ...
    if posted and show not in ("Irrelevant Only",):
        ...
        for j in posted:
            ...
            <div class="jcard">
              ...
              &nbsp;{pac_badge(cls)}
              <div style="font-size:.7rem;color:#9ca3af;margin-top:.3rem;">
                PAC model (pac_model.pkl): <b>{CLASS_LABEL[cls]}</b>
              </div>
            </div>""", unsafe_allow_html=True)
            _, b = st.columns([5, 1])
            with b:
    ...
    # Dataset listings (cleaned_jobs.csv)
    if show != "Irrelevant Only":
        ...
        for _, row in results.head(80).iterrows():
            ...
            <div class="jcard">
              ...
              <div style="font-size:.7rem;color:#9ca3af;margin-top:.3rem;">
                CSV fraudulent value: <b>{csv_lbl}</b> &nbsp;|&nbsp; pac_model.pkl prediction: <b>{CLASS_LABEL[cls]}</b>
              </div>
...
def detail_dataset(df, idx, pac_vec, model):
    ...
    <div class="detail-card">
      <div style="display:flex;align-items:center;gap:1.3rem;margin-bottom:1.3rem;">
        <div style="width:58px;height:58px;border-radius:14px;background:#0f2744;color:#fff;
          ...
          font-family:'Fraunces',serif;font-size:1.5rem;font-weight:900;flex-shrink:0;">
      ...
      <div style="background:#f5f3ef;border-radius:10px;padding:.8rem 1.1rem;margin-bottom:1rem;">
        {fraud_pill(csv_lbl)}
        <div style="font-size:.77rem;color:#6b6560;margin-top:.35rem;">
          CSV <code>fraudulent</code> column = <b>{csv_lbl}</b>
          &nbsp;→&nbsp; {'🟢 Real Job' if csv_lbl==0 else '🔴 Fake Job'}
          &emsp;|&emsp;
          <b>pac_model.pkl</b> output = <b>{cls}</b> → <b>{CLASS_LABEL[cls]}</b>
        </div>
      </div>
    ...
    if cls == 1:
        st.markdown(
            '<div class="box-warn">⚠️ <b>Blocked:</b> pac_model.pkl classified this as FAKE. '
            'Applications are disabled for your safety.</div>', unsafe_allow_html=True)
    elif cls == 2:
        st.markdown(
            '<div class="box-irr">❓ Classified as Irrelevant — applications unavailable.</div>',
            unsafe_allow_html=True)
    elif user and user["role"] == "seeker":
        if ds_applied(idx, user["id"]):
            st.markdown('<div class="box-ok">✅ You have already applied.</div>', unsafe_allow_html=True)
        else:
        ...
        st.markdown(
            '<div class="box-info">Please <b>login as a Job Seeker</b> to apply.</div>',
            unsafe_allow_html=True)
...
def detail_irrelevant(job):
    ...
    st.markdown(
        '<div class="box-irr">❓ Applications not available for irrelevant listings.</div>',
        unsafe_allow_html=True)
...
def detail_posted(j, pac_vec, model):
    ...
    <div class="detail-card">
      <div style="display:flex;align-items:center;gap:1.3rem;margin-bottom:1.3rem;">
        ...
        <div>
          <h2>{j['title']}</h2>
          <p style="color:#6b6560;font-size:.88rem;margin:0;">
            {j['company_name']} &nbsp;·&nbsp; {j['location'] or 'Remote'}
          </p>
        </div>
      </div>
      <div style="margin-bottom:.9rem;">
      ...
      <div style="background:#f5f3ef;border-radius:10px;padding:.75rem 1.1rem;margin-bottom:1rem;font-size:.78rem;color:#6b6560;">
        🤖 <b>pac_model.pkl</b> live prediction: <b>{cls}</b> → <b>{CLASS_LABEL[cls]}</b>
      </div>
      <hr style="border:none;border-top:1px solid #e2ddd6;margin:1rem 0;">
      <h4 style="color:#0f2744;margin-bottom:.4rem;">Description</h4>
    ...
    if cls == 1:
        st.markdown(
            '<div class="box-warn">⚠️ pac_model.pkl flagged this as FAKE. Applications blocked.</div>',
            unsafe_allow_html=True)
        return
    ...
    if user and user["role"] == "seeker":
        if st.button("✅ Apply Now", use_container_width=True, key="apply_posted"):
            c = _conn()
            try:
                c.execute("INSERT INTO applications(job_id,seeker_id) VALUES(?,?)",
                          (j["id"], user["id"]))
                c.commit(); st.success("🎉 Application submitted!")
    ...
    if role == "Job Seeker":
        ...
        with st.form("rc"):
            ...
            industry = st.selectbox("Industry",
                ["IT / Software","Finance","Healthcare","Manufacturing","Education","E-commerce","Other"])
            year     = st.number_input("Year Founded",
                min_value=1900, max_value=date.today().year, value=2010)
...
def page_dash_seeker():
    ...
    s = sk_get(u["id"]); total, _ = sk_stats(u["id"]); sc = profile_score(s)
    ...
    with st.sidebar:
        ...
        st.divider()
        if st.button("💼 Browse Jobs"): st.session_state.sel_job = None; go("jobs")
...
def page_dash_company():
    ...
    if section == "📊 Overview":
        ...
        jobs = co_jobs(u["id"])[:5]
        if not jobs: st.info("No jobs yet. Use ➕ Post a Job.")
        else: st.table([{"Title": j["title"], "Location": j["location"] or "Remote",
                          "Posted": j["created_at"][:10], "Applicants": j["appl"]} for j in jobs])

    elif section == "➕ Post a Job":
        st.markdown("#### Post a New Job")
        st.markdown('<div class="box-ai">🤖 pac_model.pkl will classify your job as Real, Fake, or Irrelevant automatically.</div>',
                    unsafe_allow_html=True)
        with st.form("pj"):
            c1, c2 = st.columns(2)
            title  = c1.text_input("Job Title *")
            jtype  = c2.selectbox("Type", ["Full-time","Part-time","Remote","Internship","Contract"])
            c3, c4 = st.columns(2)
            loc    = c3.text_input("Location")
            salary = c4.text_input("Salary Range")
        ...
        if sub:
            if not title or not desc: st.error("Title and description required.")
                ...
                job_post(u["id"], title, jtype, loc, salary, exp, desc, req, mob)
        ...
        if not jobs: st.info("No jobs yet.")
            ...
            for j in jobs:
                ...
                c1.markdown(f"**{j['title']}** — {j['location'] or 'Remote'}\n"
                             f"`{j['job_type']}` · {j['appl']} applicants")
                c2.caption(j["created_at"][:10])
                if c3.button("🗑", key=f"del_{j['id']}"):
                    job_delete(j["id"], u["id"]); st.success("Deleted."); st.rerun()

    elif section == "👥 Applicants":
        jobs = co_jobs(u["id"]); jmap = {"All Jobs": None}
        for j in jobs: jmap[j["title"]] = j["id"]
        chosen = st.selectbox("Filter by job", list(jmap.keys()))
        appl   = co_applicants(u["id"], jmap[chosen])
        if not appl: st.info("No applicants yet.")
        else: st.table([{"Name": a["name"], "Email": a["email"],
                          "Skills": a["skills"] or "—",
                          "Exp": f"{a['experience'] or 0} yrs",
                          "Job": a["job_title"],
                          "Applied": a["applied_at"][:10]} for a in appl])

    elif section == "🏢 Profile":
        co = co_get(u["id"])
        with st.form("cp"):
            c1, c2   = st.columns(2)
            name     = c1.text_input("Company Name", value=co.get("name","") or "")
            industry = c2.selectbox("Industry",
                ["IT / Software","Finance","Healthcare","Manufacturing","E-commerce","Other"])
            c3, c4   = st.columns(2)
            website  = c3.text_input("Website",      value=co.get("website","") or "")
            year     = c4.number_input("Year Founded",
                min_value=1900, max_value=date.today().year, value=int(co.get("year_founded") or 2010))
