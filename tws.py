...
def navbar():
    ...
    # Admin link (hidden — only visible if you know)
    with st.sidebar:
        if st.button("🔐 Admin", key="n_admin_sb"):
            go("admin")
...
# ─────────────────────────────────────────────────────────────────────────────
ADMIN_SECRET = "truehire@admin2024"   # ← change this to your own secret

def page_admin():
    st.markdown("""
    <div style="background:linear-gradient(135deg,#1a1a2e,#16213e);border-radius:18px;
    padding:2rem 2.5rem;color:#fff;margin-bottom:1.5rem;">
      <h2 style="font-family:'Fraunces',serif;margin:0 0 .3rem;font-size:1.8rem;">
        🔐 Admin Dashboard
      </h2>
      <p style="opacity:.7;margin:0;font-size:.9rem;">Full database access — TrueHire Portal</p>
    </div>""", unsafe_allow_html=True)
    ...
    if not st.session_state.get("admin_auth"):
        ...
        with ac:
            ...
            border:1px solid #e2ddd6;box-shadow:0 4px 20px rgba(0,0,0,.08);
            text-align:center;">
              <div style="font-size:2.5rem;margin-bottom:.5rem;">🔑</div>
              <h3 style="font-family:'Fraunces',serif;color:#0f2744;margin-bottom:.2rem;">Admin Access</h3>
              <p style="color:#6b6560;font-size:.85rem;margin-bottom:1.2rem;">Enter the admin secret key to continue</p>
            </div>""", unsafe_allow_html=True)
            key = st.text_input("Admin Secret Key", type="password",
                                 placeholder="Enter secret key…", key="admin_key_input")
            if st.button("🔓 Unlock Admin Panel", use_container_width=True, key="admin_unlock"):
                if key == ADMIN_SECRET:
                    st.session_state.admin_auth = True
                    st.success("✅ Access granted!")
                    st.rerun()
                else:
                    st.error("❌ Wrong secret key. Access denied.")
        return

    # Logout admin
    ac1, ac2 = st.columns([6,1])
    ac1.markdown(f"**Logged in as:** `Admin` &nbsp;|&nbsp; DB: `{DB_PATH}`")
    if ac2.button("🚪 Lock", key="admin_lock"):
    ...
    n_sk  = c.execute("SELECT COUNT(*) FROM seekers").fetchone()[0]
    n_jo  = c.execute("SELECT COUNT(*) FROM jobs").fetchone()[0]
    n_ap  = c.execute("SELECT COUNT(*) FROM applications").fetchone()[0]
    n_dsa = c.execute("SELECT COUNT(*) FROM ds_applications").fetchone()[0]
    c.close()

    st.markdown(f"""
    <div class="stat-row">
      <div class="stat-card"><div class="n">{n_co}</div><div class="l">Companies</div></div>
      <div class="stat-card"><div class="n">{n_sk}</div><div class="l">Seekers</div></div>
      <div class="stat-card"><div class="n">{n_jo}</div><div class="l">Posted Jobs</div></div>
      <div class="stat-card"><div class="n">{n_ap}</div><div class="l">Applications</div></div>
      <div class="stat-card"><div class="n">{n_dsa}</div><div class="l">DS Applications</div></div>
    </div>""", unsafe_allow_html=True)

    # Table selector
    table = st.selectbox("📋 Select Table to View",
        ["companies","seekers","jobs","applications","ds_applications","— Custom SQL Query —"],
        key="admin_table")

    c = _conn()

    if table == "— Custom SQL Query —":
        st.markdown("#### 💻 Custom SQL Query")
        st.markdown('<div class="box-warn">⚠️ Read-only recommended. Be careful with DELETE/UPDATE.</div>',
                    unsafe_allow_html=True)
        sql = st.text_area("SQL Query", height=100,
                            placeholder="SELECT * FROM seekers LIMIT 10",
                            key="admin_sql")
        col1, col2 = st.columns([1,4])
        if col1.button("▶ Run Query", key="admin_run", use_container_width=True):
            if sql.strip():
                try:
                    rows = c.execute(sql).fetchall()
                    if rows:
                        cols_names = [d[0] for d in c.execute(sql).description]
                        df_res = pd.DataFrame([dict(r) for r in rows])
                        st.success(f"✅ {len(df_res)} row(s) returned")
                        st.dataframe(df_res, use_container_width=True)
                        # Download
                        csv_data = df_res.to_csv(index=False)
                        col2.download_button("⬇ Download CSV", csv_data,
                                              file_name="query_result.csv",
                                              mime="text/csv", key="admin_dl_sql")
                        if sql.strip().upper().startswith(("INSERT","UPDATE","DELETE","DROP","CREATE")):
                            c.commit()
                    else:
                        if sql.strip().upper().startswith(("INSERT","UPDATE","DELETE")):
                            c.commit(); st.success("✅ Query executed. No rows returned.")
                        else:
        ...
        # Search within table
        search_col = st.text_input(f"🔍 Filter {table} rows", placeholder="Type to filter…",
                                    key=f"admin_search_{table}")

        try:
            all_rows = c.execute(f"SELECT * FROM {table} ORDER BY id DESC").fetchall()
            if all_rows:
                df_t = pd.DataFrame([dict(r) for r in all_rows])

                # Mask password hashes
                if "password_hash" in df_t.columns:
                    df_t["password_hash"] = "••••••••" + df_t["password_hash"].str[-4:]

                # Filter
                if search_col.strip():
                    mask = df_t.astype(str).apply(
                        lambda col: col.str.contains(search_col, case=False, na=False)
                    ).any(axis=1)
                    df_t = df_t[mask]

                st.markdown(f"Showing **{len(df_t)}** row(s)")
                st.dataframe(df_t, use_container_width=True, height=420)

                # Download
                csv_data = df_t.to_csv(index=False)
                st.download_button(f"⬇ Download {table}.csv", csv_data,
                                    file_name=f"{table}.csv", mime="text/csv",
                                    key=f"admin_dl_{table}")

                # Row-level delete
                st.markdown("#### 🗑 Delete a Row")
                del_id = st.number_input("Enter row ID to delete", min_value=1, step=1,
                                          key=f"admin_del_id_{table}")
                d1, d2 = st.columns([1,5])
                if d1.button("🗑 Delete Row", key=f"admin_del_{table}", use_container_width=True):
                    try:
                        c.execute(f"DELETE FROM {table} WHERE id=?", (int(del_id),))
                        c.commit()
                        st.success(f"✅ Row {del_id} deleted from `{table}`.")
                        st.rerun()
                    except Exception as ex:
                        st.error(f"❌ {ex}")
            else:
            ...
            st.error(f"❌ Error reading table: {ex}")

    # Raw export — all tables as one ZIP
    st.divider()
    st.markdown("#### 📦 Export Entire Database")
    if st.button("⬇ Export All Tables as CSV", key="admin_export_all", use_container_width=False):
        import io, zipfile
        tables = ["companies","seekers","jobs","applications","ds_applications"]
        buf = io.BytesIO()
            ...
            for tbl in tables:
                try:
                    ...
                    if rows:
                        ...
                        if "password_hash" in df_exp.columns:
                            df_exp["password_hash"] = "HIDDEN"
                        zf.writestr(f"{tbl}.csv", df_exp.to_csv(index=False))
                except: pass
        buf.seek(0)
        st.download_button("📥 Download truehire_db_export.zip", buf,
